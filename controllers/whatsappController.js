...
// Quick-reply generico para cualquier pregunta de texto libre (hora, fecha,
...
// Pregunta de texto libre con un boton "Cancelar" en vez del viejo pie de
// texto '(Escribi "salir" para cancelar)' -- el "cancelar" tipeado sigue
// funcionando igual (es uno de los alias del handler global), el boton es
// solo una forma mas comoda de mandarlo.
async function preguntaConCancelar(numero, texto) {
...
// ── Deteccion automatica de dias con datos incompletos (opcion 4 del menu
// de correccion) ──

// Distancia circular en minutos (0-1439): un turno que cruza medianoche
// (ej Aaron Garcen 22:00-06:00) no debe dar una distancia gigante entre una
// hora de la noche y una de la madrugada que en la practica estan pegadas.
...
// Horario esperado (entrada/salida en minutos) para un empleado en una
// fecha puntual -- mantenimiento usa la rotacion real de ese dia,
// conserjeria su turno fijo. Null si ese dia no tenia turno asignado
// (franco/descanso): ahi no hay con que comparar.
function horarioEsperadoDelDia(empleado, fechaISOtexto) {
  const [y, m, d] = fechaISOtexto.split("-").map(Number);
  const fechaObj = new Date(y, m - 1, d);

  if (esDelEquipo(empleado)) {
    const turno = turnoRealDelDia(empleado, fechaObj);
    if (!turno || !turno.horario) return null;
    return { in: minutosDeHora(turno.horario.in), out: minutosDeHora(turno.horario.out) };
  }

  const tf = TURNOS_FIJOS_CONSERJERIA[empleado];
  if (!tf) return null;
  return { in: tf.inMin, out: tf.outMin };
}

// Para un dia con un unico horario cargado (el otro esta en "—"), decide
// que campo falta DE VERDAD. El pipeline por default siempre guarda ese
// horario suelto como "ingreso" (ver motorCalculo.procesarRegistros, el
// alerta "Solo 1 registro" no distingue si en realidad fue una salida) --
// asi que no alcanza con mirar como quedo etiquetado, hay que comparar
// contra el turno esperado ese dia para saber si en realidad es al reves.
function campoQueFaltaDeVerdad(empleado, fila) {
  const campoConDato = fila.ingreso !== "—" ? "ingreso" : "egreso";
  const valorConocido = campoConDato === "ingreso" ? fila.ingreso : fila.egreso;
  const campoVacio = campoConDato === "ingreso" ? "egreso" : "ingreso";

  const horario = horarioEsperadoDelDia(empleado, fila.fecha);
  let heuristico = campoConDato; // sin turno de referencia, se confia en la etiqueta que ya tiene
  if (horario) {
    const minutos = minutosDeHora(valorConocido);
    const distIn = distanciaCircular(minutos, horario.in);
    const distOut = distanciaCircular(minutos, horario.out);
    heuristico = distIn <= distOut ? "ingreso" : "egreso";
  }

  // Si el horario suelto coincide con como esta guardado, lo que falta es
  // el otro extremo del dia (caso normal). Si NO coincide, el horario
  // suelto en realidad ocupa el rol contrario al que tiene guardado -- lo
  // que falta de verdad es el mismo campo donde hoy esta ese dato (al
  // pedirlo y guardarlo, la salvaguarda de aplicarCorreccion reacomoda solo
  // el valor viejo al campo correcto).
  const campoAPedir = heuristico === campoConDato ? campoVacio : campoConDato;
  return { campoAPedir, heuristico, campoConDato, valorConocido };
}

// Dias del periodo actual del empleado con exactamente un horario cargado.
// No incluye dias con 3+ marcaciones sueltas (esos ya quedan marcados
// "REVISAR MANUALMENTE", son un caso mas raro para que el admin mire a
...
function diasIncompletosDetectados(empleado) {
  ...
  return filasDeEmpleadoPorFecha(empleado, desde, hasta)
    ...
    .map((f) => ({ fecha: f.fecha, ...campoQueFaltaDeVerdad(empleado, f) }));
}
...
async function mostrarDiaActual(numero, pendientes, resueltos) {
  ...
  const contador = pendientes.length > 1 ? `_Quedan ${pendientes.length} días con datos incompletos._\n\n` : "";
  const texto = `${contador}${formatoDiaMes(d.fecha)} — te falta ${campoFaltante} (tenés registrada ${rolConocido} ${d.valorConocido})`;
...
async function procesarMensajeEmpleado(empleado, numero, textoOriginal) {
  ...
  switch (conv.estado) {
    ...
    // Respuesta a la lista de dias detectados automaticamente (opcion 4).
    // Cada renglon es "DD/MM HH:MM" (un solo horario, va al campo que
    ...
    case "correccion:otro-dia": {
      ...
      if (!fecha) {
        return await preguntaConCancelar(numero, "Mandame una fecha (DD/MM) para corregir un día puntual.");
      }
    ...
    case "licencia:fecha-motivo": {
      ...
      if (fechaHasta < fechaDesde) {
        return await preguntaConCancelar(numero, `La fecha de fin es anterior al ${formatoDiaMes(fechaDesde)}. Mandá el rango de nuevo.`);
      }
      if (!motivo) {
...
// Aviso de que una solicitud fue aprobada/rechazada -- lo manda el bot por
// su cuenta (el empleado no acaba de escribir), asi que si hace mas de 24hs
// que no habla con el bot, WhatsApp no deja el texto libre especifico
// (textoLibre) y hay que caer a la Content Template generica ya aprobada.
// Pierde algo de detalle en ese caso, pero es mejor a no avisar nada.
// Nunca tira error: es un aviso, no la accion en si (aprobar/rechazar ya
// quedo aplicado en la base antes de llamar a esto). Si el envio falla --
// plantilla todavia no aprobada por Meta, numero invalido, lo que sea -- se
// loguea y se sigue, para no romper la respuesta de "aprobar #N"/"rechazar
// #N" en el panel o por WhatsApp solo porque el aviso no pudo salir.
async function avisarResultado(numero, nombreEmpleado, tipo, resultado, textoLibre) {
  try {
    await enviarWhatsappVentana(
      numero, textoLibre,
      process.env.CONTENT_SID_AVISO_RESULTADO,
      { "1": nombreEmpleado.split(" ")[0], "2": `tu solicitud de ${tipo} fue ${resultado}` }
    );
  } catch (err) {
    console.error(`No se pudo avisar a ${numero} (${tipo} ${resultado}):`, err.message);
  }
}
