...
// Describe un dia marcado con alerta. Dos alertas muy distintas comparten
// la lista de "incompletos", asi que hay que distinguirlas:
//  - "REVISAR MANUALMENTE": el dia SI tiene entrada y salida (y las horas ya
//    estan calculadas) — la alerta es porque hubo mas de 2 fichajes ese dia
//    y el sistema no puede saber cual par es el real, no porque falte un dato.
//  - Cualquier otra: genuinamente falta un horario (o los dos). El "—" del
//    excel viejo tambien cuenta como faltante (no es una hora real). Cuando
//    el otro dato SI esta cargado, lo mostramos (ej: "entrada 08:00, falta salida").
function descripcionAlerta(fila) {
  if (fila.alerta && fila.alerta.includes("REVISAR MANUALMENTE")) {
    return "revisar manualmente (hubo más de 2 fichajes ese día, no está claro cuál es el real)";
  }
  const ingresoOk = fila.ingreso && parsearHora(fila.ingreso);
  const egresoOk = fila.egreso && parsearHora(fila.egreso);
  if (!ingresoOk && !egresoOk) return "falta entrada y salida";
  if (!ingresoOk) return `salida ${fila.egreso}, falta entrada`;
  if (!egresoOk) return `entrada ${fila.ingreso}, falta salida`;
  return "falta un dato"; // no deberia pasar si hay alerta, pero por las dudas
}

// Una linea por dia, con la misma forma siempre (🟢entrada → 🔴salida) para
// que sea facil de escanear de arriba a abajo — si falta un dato se ve un
// ❌ en su lugar en vez de cambiar toda la frase. "Revisar manualmente" es
// el unico caso realmente distinto (ahi SI hay entrada/salida calculadas,
// el problema es otro), asi que tiene su propio icono y va corto.
function lineaDia(fila) {
  const fechaDisplay = formatoDiaMes(fila.fecha);

  if (fila.alerta && fila.alerta.includes("REVISAR MANUALMENTE")) {
    return `📅 ${fechaDisplay}  🔍 revisar manualmente`;
  }
  ...
  const extras = [];
  if (fila.h50 > 0) extras.push(`⚠️+${fila.h50}h 50%`);
  if (fila.h100 > 0) extras.push(`🔥+${fila.h100}h 100%`);
  const extraTexto = extras.length > 0 ? `  ${extras.join("  ")}` : "";

  return `📅 ${fechaDisplay}  ${ingresoTxt} → ${egresoTxt}  ⏱️${fila.total_hs}h${extraTexto}`;
}
...
function mensajeHoras(empleado) {
  ...
  const hayRevisar = filas.some((f) => f.alerta && f.alerta.includes("REVISAR MANUALMENTE"));
  const hayFaltantes = filas.some((f) => f.alerta && !f.alerta.includes("REVISAR MANUALMENTE"));

  let notas = "";
  if (hayRevisar) notas += "\n🔍 = varios fichajes ese día, no está claro cuál es el real";
  if (hayFaltantes) notas += "\n❌ = falta ese dato";
  if (hayRevisar || hayFaltantes) notas += '\n(pedí la corrección con la opción 2)';

  return (
    `Hola ${empleado.split(" ")[0]} 👋\n` +
    `🗓️ Período: *${rangoPeriodo}*\n\n` +
    detalle +
    `\n\n📊 *Totales*\n` +
    `📅 Días trabajados: *${resumen.dias}*\n` +
    `⏱️ Horas totales: *${resumen.total_hs}*\n` +
    `⚠️ Extra 50%: *${resumen.h50}*\n` +
