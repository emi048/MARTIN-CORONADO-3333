...
function lineaDia(fila) {
  ...
  if (extras.length === 0) {
    return `° ${fechaDisplay} ${ingresoTxt} -- ${egresoTxt} (${fila.total_hs}hs)`;
  }

  // Cuando hay horas extra la linea se hace larga y en el celular WhatsApp la
  // corta a la mitad, dejando un "50%)" huerfano colgando solo en el renglon
  // siguiente. Se fuerza el salto de linea nosotros mismos, con sangria, para
  // que quede prolijo en vez de partido al azar por el ancho de pantalla.
  return `° ${fechaDisplay} ${ingresoTxt} -- ${egresoTxt}\n   ${fila.total_hs}hs total (${extras.join(", ")})`;
}
