...
const { mensajeListaTareas, marcarTareaHechaYNotificar } = require("./tareas");
...
const MENU_TEXT =
  ...
  'Escribí *Lista* para ver tus tareas pendientes.';
...
async function procesarMensajeEmpleado(empleado, numero, textoOriginal, media) {
  ...
  // "Lista" y "tarea N finalizada" son comandos globales -- funcionan desde
  // cualquier punto del menu, no solo desde "menu", para no obligar a
  // interrumpir un pedido de correccion en curso para cerrar una tarea.
  if (textoLower === "lista" || textoLower === "tareas") {
    registrarMensaje(numero, empleado, "lista_tareas");
    return mensajeListaTareas(empleado);
  }

  const matchTareaHecha = texto.match(/^tarea\s+(\d+)\s+finalizad[ao]$/i);
  if (matchTareaHecha) {
    registrarMensaje(numero, empleado, "tarea_finalizada");
    const resultado = await marcarTareaHechaYNotificar({
      id: Number(matchTareaHecha[1]),
      empleado,
      mediaUrl: media && media.numMedia > 0 ? media.mediaUrl : null,
      mediaContentType: media && media.mediaContentType,
    });
    if (resultado.error) return resultado.error;
    const notaFoto = resultado.tieneFoto ? " Se guardó la foto que mandaste." : "";
    const notaPlazo = resultado.fueraDePlazo ? " (fuera de plazo, pero queda registrado)" : "";
    return `✅ Tarea #${matchTareaHecha[1]} marcada como finalizada${notaPlazo}.${notaFoto}`;
  }
...
function manejarAltaEmpleado(texto) {
  ...
  // Mismo chequeo que el panel (POST /api/empleados/whatsapp) -- el nombre
  // tiene que ser uno del roster fijo (motorCalculo.js), esto no da de alta
  // un empleado nuevo que no exista ahi, solo gestiona su acceso al bot.
  if (!todosLosEmpleados().includes(empleado)) {
    return `No encontré "${empleado}" en la lista de empleados. Revisá que el nombre esté escrito igual que en el sistema.`;
  }
...
router.post("/webhook", express.urlencoded({ extended: false }), async (req, res) => {
  ...
  try {
    ...
    const media = numMedia > 0 ? { numMedia, mediaUrl: req.body.MediaUrl0, mediaContentType: req.body.MediaContentType0 } : null;
    twiml.message(await procesarMensajeEmpleado(empleado, numero, texto, media));
  } catch (err) {
