...
// su lugar en vez de cambiar toda la frase. "Revisar manualmente" es el
// unico caso realmente distinto (ahi SI hay entrada/salida calculadas, el
// problema es otro), asi que va corto y aparte.
function lineaDia(fila) {
  ...
  const extras = [];
  if (fila.h50 > 0) extras.push(`+${fila.h50}h 50%`);
  if (fila.h100 > 0) extras.push(`+${fila.h100}h 100%`);
  const extraTexto = extras.length > 0 ? `  (${extras.join(", ")})` : "";

  return `${fechaDisplay}  ${ingresoTxt} - ${egresoTxt}  ${fila.total_hs}h${extraTexto}`;
}
...
function mensajeHoras(empleado) {
  ...
  let notas = "";
  if (hayRevisar) notas += "\n\"revisar manualmente\": varios fichajes ese día, no está claro cuál es el real";
  if (hayFaltantes) notas += "\n\"sin dato\": falta ese horario";
  if (hayRevisar || hayFaltantes) notas += "\nPedí la corrección con la opción 2 si corresponde.";
