...
function mensajeHoras(empleado) {
  ...
  const lineaAlertas = diasIncompletos.length > 0
    ? `\n\n⚠️ Días a revisar (pedí la corrección con la opción 2 si falta un horario):\n` +
      diasIncompletos.map((f) => `• ${formatoDiaMes(f.fecha)} — ${descripcionAlerta(f)}`).join("\n")
    : "";
...
function recalcularHorasDia({ empleado, fecha, ingreso, egreso }) {
  ...
  return calcularHoras(fechaObj, dtEntrada, dtSalida, cfg);
}
