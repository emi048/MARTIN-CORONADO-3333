...
const { turnoDelDia, esDelEquipo, GRUPO_A, GRUPO_B } = require("./turnosMantenimiento");
...
function lineaDia(fila) {
  ...
  // Doble guion entre las horas (no uno solo) — WhatsApp a veces interpreta
...
async function crearSolicitudCambioYNotificar(empleadoA, numeroA, empleadoB, fechaA, fechaB) {
  ...
  const turnoA = turnoDelDia(empleadoA, new Date(yA, mA - 1, dA));
  const turnoB = turnoDelDia(empleadoB, new Date(yB, mB - 1, dB));
  ...
  switch (conv.estado) {
    case "menu": {
      ...
      if (texto === "5" && esDelEquipo(empleado)) {
        ...
        return (
          "¿Qué día tuyo querés ceder? Formato DD/MM.\nEj: 19/7" +
    ...
    case "cambio:fecha-propia": {
      ...
      if (!fecha) return "Ese formato no lo pude leer. Mandá la fecha como DD/MM (ej: 19/7), o \"salir\" para cancelar.";
      const [y, m, d] = fecha.split("-").map(Number);
      const turnoPropio = turnoDelDia(empleado, new Date(y, m - 1, d));
      if (!turnoPropio || turnoPropio.tipo === "franco" || turnoPropio.tipo === "descanso") {
        return `Ese día (${formatoDiaMes(fecha)}) ya lo tenés libre, no hay turno para ceder. Probá con otra fecha, o "salir" para cancelar.`;
      }
    ...
    case "cambio:companero": {
      ...
      return `¿Qué día de ${companero.split(" ")[0]} tomás vos a cambio? Formato DD/MM.` + '\n\n(Escribí "salir" para cancelar)';
    }

    case "cambio:fecha-companero": {
      const fechaB = parsearFecha(texto);
      if (!fechaB) return "Ese formato no lo pude leer. Mandá la fecha como DD/MM, o \"salir\" para cancelar.";
      const { fechaA, companero } = conv.datos;
      const [y, m, d] = fechaB.split("-").map(Number);
      const turnoCompanero = turnoDelDia(companero, new Date(y, m - 1, d));
...
async function resolverUnaSolicitudCambio(accion, id) {
  ...
  const turnoA_enA = turnoDelDia(empleado_a, fA);
  const turnoB_enA = turnoDelDia(empleado_b, fA);
  const turnoA_enB = turnoDelDia(empleado_a, fB);
  const turnoB_enB = turnoDelDia(empleado_b, fB);
...
router.post("/webhook", express.urlencoded({ extended: false }), async (req, res) => {
  ...
  try {
    ...
    const esComandoAdmin = esAprobarRechazar || esAprobarRechazarCambio || esPendientes || esCambiosPendientes || esExcel || esAlta || esEvento;
