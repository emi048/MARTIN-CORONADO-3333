...
// Una linea por dia: si esta incompleto, la misma descripcion que ya se usa
// en las alertas; si esta completo, el horario real y cuanto de eso fue
// extra (si hubo). Mostrar solo el total sin el detalle no alcanza — el
// empleado tiene que poder ver de donde sale cada hora.
function lineaDia(fila) {
  const fechaDisplay = formatoDiaMes(fila.fecha);
  if (fila.alerta) {
    return `${fechaDisplay} — ${descripcionAlerta(fila)}`;
  }
  const extras = [];
  ...
  return `${fechaDisplay} ${fila.ingreso}-${fila.egreso} — ${fila.total_hs}h${extraTexto}`;
}
...
function mensajeHoras(empleado) {
  ...
  return (
    ...
    (filas.some((f) => f.alerta) ? `\n\n(Los días marcados como incompletos: pedí la corrección con la opción 2)` : "")
  );
}
