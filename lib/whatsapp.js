...
// Dice concretamente que horario falta de un dia marcado con alerta — el
// "—" del excel viejo tambien cuenta como faltante (no es una hora real).
function queFalta(fila) {
  const ingresoOk = fila.ingreso && parsearHora(fila.ingreso);
  const egresoOk = fila.egreso && parsearHora(fila.egreso);
  if (!ingresoOk && !egresoOk) return "entrada y salida";
  if (!ingresoOk) return "entrada";
  if (!egresoOk) return "salida";
  return "un dato"; // no deberia pasar si hay alerta, pero por las dudas
}
...
function mensajeHoras(empleado) {
  ...
  const diasIncompletos = filas.filter((f) => f.alerta);
  const lineaAlertas = diasIncompletos.length > 0
    ? `\n\n⚠️ Días con fichaje incompleto (pedí la corrección con la opción 2):\n` +
      diasIncompletos.map((f) => `• ${formatoDiaMes(f.fecha)} — falta ${queFalta(f)}`).join("\n")
    : "";
  ...
  switch (conv.estado) {
    ...
    case "correccion:campo": {
      ...
      return (campo === "ingreso"
        ...
        : `¿A qué hora saliste ${plural}? Formato HH:MM (ej: 18:30).`) + pieSalir;
    }

    case "correccion:hora": {
      const hora = parsearHora(texto);
      if (!hora) {
        return "Formato inválido. Mandá la hora como HH:MM (ej: 18:30), o escribí \"salir\" para cancelar.";
      }
      const datos = conv.datos;
    ...
    case "correccion:horas-ambas": {
      ...
      if (!horas) {
        return "Formato inválido. Mandá las dos horas separadas por un espacio (ej: 08:00 17:00), o escribí \"salir\" para cancelar.";
      }
      const datos = conv.datos;
...
async function aplicarCorreccion(solicitud) {
  ...
  const ingreso = ingreso_propuesto || (filaExistente && horaExistenteValida(filaExistente.ingreso));
  const egreso = egreso_propuesto || (filaExistente && horaExistenteValida(filaExistente.egreso));
...
router.post("/webhook", express.urlencoded({ extended: false }), async (req, res) => {
  ...
  try {
    ...
    const esComandoAdmin = esAprobarRechazar || esPendientes || esExcel;
