...
const { crearTareaYNotificar, reporteMensual } = require("./tareas");
const { generarReporteTareas } = require("./generarReporteTareas");
...
// Comparacion a tiempo constante: password !== env directo filtra por
// timing cuanto del prefijo coincide. Se hashean los dos lados primero para
// que timingSafeEqual siempre compare buffers del mismo largo (si no,
// tira si password y ADMIN_PANEL_PASSWORD tienen longitudes distintas).
function passwordValida(password) {
  if (!process.env.ADMIN_PANEL_PASSWORD || !password) return false;
  const hashIngresado = crypto.createHash("sha256").update(password).digest();
  const hashReal = crypto.createHash("sha256").update(process.env.ADMIN_PANEL_PASSWORD).digest();
  return crypto.timingSafeEqual(hashIngresado, hashReal);
}
...
router.get("/api/tareas", requerirAuth, (req, res) => {
  const desde = /^\d{4}-\d{2}-\d{2}$/.test(req.query.desde || "") ? req.query.desde : primerDiaDelMes();
  const hasta = /^\d{4}-\d{2}-\d{2}$/.test(req.query.hasta || "") ? req.query.hasta : hoyISO();
  res.json({ ...reporteMensual(desde, hasta), desde, hasta, empleados: todosLosEmpleados() });
});

router.post("/api/tareas", requerirAuth, async (req, res) => {
  const { titulo, descripcion, plazoHoras, asignadoA } = req.body || {};

  if (!titulo || !titulo.trim()) return res.status(400).json({ ok: false, error: "Falta el título" });
  const plazo = Number(plazoHoras);
  if (!Number.isFinite(plazo) || plazo <= 0 || plazo > 24 * 90) {
    return res.status(400).json({ ok: false, error: "Plazo inválido (en horas, hasta 90 días)" });
  }
  if (asignadoA && !todosLosEmpleados().includes(asignadoA)) {
    return res.status(400).json({ ok: false, error: "Empleado inválido" });
  }

  try {
    const resultado = await crearTareaYNotificar({
      titulo: titulo.trim(), descripcion: (descripcion || "").trim(), plazoHoras: plazo,
      asignadoA: asignadoA || null, creadoPor: "panel",
    });
    res.json({ ok: true, ...resultado });
  } catch (err) {
    res.status(500).json({ ok: false, error: err.message });
...
router.get("/api/tareas/excel", requerirAuth, (req, res) => {
  const desde = /^\d{4}-\d{2}-\d{2}$/.test(req.query.desde || "") ? req.query.desde : primerDiaDelMes();
  const hasta = /^\d{4}-\d{2}-\d{2}$/.test(req.query.hasta || "") ? req.query.hasta : hoyISO();
  const { tareas } = reporteMensual(desde, hasta);
  const buffer = generarReporteTareas(tareas);
  res.setHeader("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
  res.setHeader("Content-Disposition", `attachment; filename="tareas_${desde}_a_${hasta}.xlsx"`);
  res.send(buffer);
});
