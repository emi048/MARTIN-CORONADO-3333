...
// Filas de los ultimos N dias con datos, sin importar el periodo al que
// hayan quedado etiquetadas — para la pestaña de turnos, que muestra el
// calendario real (dia por dia), no el agrupado por periodo.
function filasRecientes(limiteDias = 14) {
  const fechas = db.prepare(`
    SELECT DISTINCT fecha FROM filas_diarias ORDER BY fecha DESC LIMIT ?
  `).all(limiteDias).map(r => r.fecha);
  if (fechas.length === 0) return [];
  const placeholders = fechas.map(() => "?").join(",");
  return db.prepare(`
    SELECT * FROM filas_diarias WHERE fecha IN (${placeholders}) ORDER BY fecha DESC, empleado ASC
  `).all(...fechas);
}
...
module.exports = {
  ...
  guardarFilasDiarias, filaDelDia, filaDelDiaPorFecha, filasDelPeriodo, filasDelPeriodoDeEmpleado, filasRecientes, actualizarFilaDiaria, recalcularResumenEmpleado,
