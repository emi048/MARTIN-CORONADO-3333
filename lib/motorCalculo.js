...
const TURNOS_FIJOS_CONSERJERIA = {
  ...
  "Veronica Montenegro":    { turno: "Tarde",  inMin: 14 * 60, outMin: 22 * 60, horasDia: 8, horasSab: 4 },
...
function calcularHoras(fecha, dtEntrada, dtSalida, cfg, turnoAsignado) {
  ...
  if (esFeriado) {
    ...
    if (turnoAsignado) {
      const inMins  = cfg.mañanaIn;
      const outMins = cfg.mañanaOut;
      const asigIn  = new Date(fecha);
      asigIn.setHours(Math.floor(inMins / 60), inMins % 60, 0, 0);
      let asigOut;
      if (turnoAsignado === "Noche" && outMins < inMins) {
        asigOut = new Date(fecha);
        asigOut.setDate(asigOut.getDate() + 1);
        asigOut.setHours(Math.floor(outMins / 60), outMins % 60, 0, 0);
      } else {
        asigOut = new Date(fecha);
        asigOut.setHours(Math.floor(outMins / 60), outMins % 60, 0, 0);
      }
      const startReg = Math.max(dtEntrada, asigIn);
      const endReg   = Math.min(dtSalida,  asigOut);
      let hsRegulares = 0;
      if (startReg < endReg) hsRegulares = (endReg - startReg) / 3600000;
      h50 = Math.max(0, totalHs - hsRegulares);
    } else {
      h50 = Math.max(0, totalHs - cfg.horasDia);
    }
  }
...
function procesarRegistros(registros, rotacionInicialMañana = "A") {
  ...
  Object.values(mapa).forEach(grupo => {
    ...
    const esConserjeria = !!turnoFijo;
    const turnoParaCalculo = esConserjeria ? turnoAsignadoStr : null;
    const calc = n >= 2 ? calcularHoras(grupo.fecha, dtEntrada, dtSalida, cfgEmpleado, turnoParaCalculo) : null;
