...
async function correrPipelineMensual({ fechaDesde, fechaHasta, registrosManuales = [] } = {}) {
  ...
  // a quien lee el mail es el rango real de dias que se procesaron.
  const fechasReales = filas.map((f) => f._fechaReal).sort((a, b) => a - b);
  const formatoCorto = (d) => `${d.getDate()}/${d.getMonth() + 1}`;
  const rangoTexto = fechasReales.length > 0
    ? `${formatoCorto(fechasReales[0])} al ${formatoCorto(fechasReales[fechasReales.length - 1])}`
    : periodo;
