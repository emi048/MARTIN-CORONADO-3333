...
const EVENTOS = new Set([
  ...
  "Alberto Reynoso|2026-07-16",
]);

// Los eventos ya cargados a mano en el codigo (arriba) se mantienen, y se
// suman los que se van cargando en caliente por WhatsApp (comando "evento",
// tabla eventos en la base) sin necesitar un deploy para cada uno.
function esDiaDeEvento(empleado, fechaISO) {
  if (EVENTOS.has(empleado + "|" + fechaISO)) return true;
  const { esEventoRegistrado } = require("./db");
  return esEventoRegistrado(empleado, fechaISO);
}
